...
| 6 | H2 批2 交付（RBAC 注册中心 + 权限/角色表 + 权限门控中间件 + /roles /permissions /roles/groups 端点 + 16 个单测；/users CRUD 未含） | ✅ commit `987…` |
| 7 | 后续：H2 批2b（真实 /users CRUD + 角色分配 UI）/ H3 文档对齐（Agent 改"两层架构"） | ⏳ 待用户决策 |
